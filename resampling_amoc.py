...
print("Read amoc successfully")
### ORA FAI IL RESAMPLING
W=np.exp(k*dt(obsmean-np.mean(obsmean))) # TOGLIE MEDIA PER QUESTIONE NUMERICA, EVITARE ESPONENZIALI ALTI
#exponent -= np.max(exp)
#W=np.exp(exponent)
#W=np.exp(k*(-obsmean+np.mean(obsmean))) # LOOK FOR MINIMUM
#change weighting so infinites stop happening
#exponent=k*(-obsmean+np.mean(obsmean))
...
np.savez(resamplingexpdir+'/block_'+str(block).zfill(4)+'/'+expname+'_resampling.'+str(block).zfill(4),\
         ntrajs=ntrajs,Nc=Nc,dN=dN,R=R,W=W,w=w,nc=nc,killedlabel=killedlabel,udkilledlabel=udkilledlabel,initlabel=initlabel,k=k,dt=dt,\
         obsmean=obsmean)
print("Saved resampling file to:", resamplingexpdir)
